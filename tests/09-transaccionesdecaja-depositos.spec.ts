...
test.describe('Pruebas con Transacciones de Caja - Deposito', () => {
    ...
    test('Transacciones de Caja - Depositos', async () => {
        ...
        const mensajeAviso = page.locator('text=Es necesario registrar la tasa del día. Imposible realizar operaciones');
        
         if (await mensajeAviso.isHidden()) {
            ...
            test('Datos de la Distribucion de Ingresos', async () => {
                ...
                // Hacer la distribucion del dinero a depositar, en el caso de la prueba RD 500
                // Divididos en 200, 100, 100 y 50, 50
            ...
            test('Actualizar la libreta luego de realizar el deposito', async () => {
                ...
                // La cantidad depositada debe estar visible
                await expect(newPage.locator('text=500').first()).toBeVisible();
                
                // La pagina abierta con la vista previa de la libreta se debe cerrar
                await newPage.close();
            });
        } else if (await mensajeAviso.isVisible()) {
            // skip el test si el mensaje aparece
            test.skip();
        }
    });
