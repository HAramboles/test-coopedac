...
import { url_base, EscenariosPruebasActivarInactivarCuentas, formBuscar } from './utils/dataTests';
...
// Boton de Activar/Inactivar Cuenta
...
test.describe.serial('Inactivar una Cuenta del Socio - Pruebas con los diferentes parametros', async () => {
    for (const escenario of EscenariosPruebasActivarInactivarCuentas) {
        test.describe(`Test cuando el parametro sea: ${Object.values(escenario).toString()}`, async () => {
            test.beforeAll(async () => { // Antes de las pruebas
                ...
                await page.route(/\/relation/, async route => {
                    ...
                    if (Object.keys(body?.data[27]).length > 1) { 
                        ...
                        body.data[27] = Object.assign(body.data[27], escenario);
                        route.fulfill({
                            response,
